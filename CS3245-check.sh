...
if unzip -B $zipfile -d evaluation/$studentNumber/; then
    ...
    if [ -d $path ]
    then
        echo -e "\t Attempting to find $README, $DICTIONARY, $POSTINGS in $path"
    else
        echo -e "\t\t $path does not exist, re-attempting in another path"
        path=evaluation/$studentNumber
    fi
    ...
    then
        let "submittedFiles=submittedFiles+1"
        echo -e "\t\t Found $DICTIONARY in $path"
    else
